...
for subject in subjects:
    ...
    for i in range(6):
        ...
        #svm1.fit(X_train[::subsample,:],y_train[::subsample])
        #svm2.fit(X_train[::subsample2,:],y_train[::subsample2])
        
        pred1[:,i] = lr1.predict_proba(X_test)[:,1]
        pred2[:,i] = lr2.predict_proba(X_test)[:,1]
        #pred2[:,i] = svm1.predict(X_test)
        pred3[:,i] = lr3.predict_proba(X_test)[:,1]
        #pred4[:,i] = svm2.predict(X_test)
        
        pred[:,i]=pred1[:,i]*0.24+pred2[:,i]*0.24+pred3[:,i]*0.52# + pred4[:,i]*.32
        #pred[:,i]=pred2[:,i]*0.36+ pred4[:,i]*.64
